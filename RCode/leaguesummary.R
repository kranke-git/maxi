...
routdir    <- '/home/kranke/Documents/ResearchProjects/Maxithlon/Output/LeagueTables'
colsummary <- c( 'age', 'wage', 'height', 'weight', 'fans', 'maxid', 'form', 'care', 'experience', 'mood' )
timestmp   <- '20250111_172940UTC'
# Input ends

for ( leagueid in leagueids ) {
  
  print( paste( '----Processing League', leagueid ) )

  # Get the teamids and info from the league
  leaguefile <- paste( datadir, '/LEAGUE/', timestmp, '/league_', leagueid, '.xml', sep = '' )
  ...
  for ( teamid in teamids ) {
    ...
    teamfile <- paste( datadir, '/TEAM/', timestmp, '/teaminfo_', teamid, '.xml', sep = '' )
    teamdoc  <- read_xml( teamfile )
   
    # Athletes file
    athfile  <- paste( datadir, '/ATHLETES/', timestmp, '/athletes_team_', teamid, '.xml', sep = '' )
  ...
  writeLeagueTable( leaguedf, 'mainteam', digits, paste( routdir, '/', timestmp, sep = ''), season, level, number, nationNames[ nationId ], sortedRegions, timestmp ) 
  writeLeagueTable( seniordf, 'mainteam_noyouth', digits, paste( routdir, '/', timestmp, sep = ''), season, level, number, nationNames[ nationId ], sortedRegions, timestmp ) 
  for ( s in specids ) {
    writeLeagueTable( speclst[[ s ]], paste( 'mainteam_', specialtyNames[ s ], sep = '' ), digits, paste( routdir, '/', timestmp, sep = ''), season, level, number, nationNames[ nationId ], sortedRegions, timestmp )  
  }
}
